...
bool FogShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Fog.vs", L"Fog.ps");
	if (!result)
	{
		return false;
	}
...
bool FogShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* textrue,float fogStart,float fogEnd)
{
	bool result;

	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textrue, fogStart, fogEnd);
	if (!result)
		return false;
...
FogShaderClass::FogShaderClass():m_vertexShader(nullptr),m_pixelShader(nullptr),m_layout(nullptr),m_matrixBuffer(nullptr),m_sampleState(nullptr),m_fogBuffer(nullptr)
{
}
