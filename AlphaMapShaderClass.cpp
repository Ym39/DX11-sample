...
AlphaMapShaderClass::AlphaMapShaderClass():m_vertexShader(nullptr),m_pixelShader(nullptr),m_layout(nullptr),m_matrixBuffer(nullptr),m_sampleState(nullptr)
{
}
...
bool AlphaMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Multitexture.vs", L"Alphamap.ps");
	if (!result)
	{
		return false;
	}
...
bool AlphaMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray)
{
	bool result;
...
bool AlphaMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray)
{
	HRESULT result;
