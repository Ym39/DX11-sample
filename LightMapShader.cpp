...
LightMapShader::LightMapShader():m_vertexShader(nullptr),m_pixelShader(nullptr),m_layout(nullptr),m_matrixBuffer(nullptr),m_sampleState(nullptr)
{
}
...
bool LightMapShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Multitexture.vs", L"LightMap.ps");
	if (!result)
	{
		return false;
	}
...
bool LightMapShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray)
{
	bool result;
...
bool LightMapShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray)
{
	HRESULT result;
